...
import { triggerOnboardingApologyEmails } from "@app/dashboard/admin/actions";

type RunResult = { sent: number; skipped: number; errors: number };

export function OnboardingApologyButton() {
  ...
  const handleTrigger = async () => {
    ...
    setResult(null);
    try {
      const data = await triggerOnboardingApologyEmails();
      setResult(data);
      setStatus("success");
    } catch {
      setStatus("error");
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        ...
        {status === "loading" ? "Sending..." : "Send apology emails"}
      </button>
