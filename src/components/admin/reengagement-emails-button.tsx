...
import { triggerReengagementEmails } from "@app/dashboard/admin/actions";

type RunResult = { sent: number; skipped: number; errors: number };

export function ReengagementEmailsButton() {
  ...
  const handleTrigger = async () => {
    ...
    setResult(null);
    try {
      const data = await triggerReengagementEmails();
      setResult(data);
      setStatus("success");
    } catch {
      setStatus("error");
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      >
        ...
        {status === "loading" ? "Sending..." : "Run re-engagement emails"}
      </button>
