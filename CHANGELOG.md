...
CHORE(db): add `migrations/00000000_initial_schema.sql` — the schema was never versioned. The 16 files in `migrations/` were all incremental `ALTER TABLE`s with no base to alter; a fresh clone could not create a single table, which made the project impossible to install for anyone but the author. The new file is a `pg_dump --schema-only` snapshot of production: 33 tables, 46 RLS policies, 103 indexes, 14 functions, 8 triggers, 1 view. Two objects `--schema=public` cannot see are appended by hand and commented — the `on_auth_user_created` trigger on `auth.users` (without it, sign-up succeeds and the user lands on an empty dashboard forever) and the public `audio` storage bucket with its two policies. Verified by applying it to an empty PostgreSQL 17 container against a minimal Supabase stand-in: zero errors, and an insert into `auth.users` correctly produced a `profiles` row. `migrations/README.md` documents that this is the only file to run and that the others are historical.

FIX(security): `.env.production` was not gitignored. `.gitignore` matched `.env` and `.env*.local` but nothing in between, so a single file with that name would have committed all 50 keys at once. Replaced with a deny-all (`.env`, `.env.*`, `*.env`) plus explicit allows for the templates. Verified against 8 filename variants.

FIX(security): stop hardcoding the Sentry DSN. `instrumentation-client.ts`, `sentry.server.config.ts` and `sentry.edge.config.ts` each embedded the upstream project's DSN, so every self-hosted instance would have shipped its users' errors to the maintainer's account without anyone noticing. The DSN now comes from `NEXT_PUBLIC_SENTRY_DSN` (server side: `SENTRY_DSN` with fallback), and `Sentry.init()` is skipped entirely when it is absent — no DSN, no data leaving the instance. **Action required before the next production deploy: set `NEXT_PUBLIC_SENTRY_DSN` in Vercel/Infisical, otherwise error reporting silently stops.**

FIX(ci): `playwright.yml` fell back to the production Supabase project ref and anon key when repository secrets were absent. The keys are public by design, but the fallback meant a manually-triggered run from a fork would have pointed straight at production data. Replaced with neutral placeholders. Also bumped `pnpm/action-setup@v2` with `version: 9` to `@v4` with no version — `package.json` pins `pnpm@10.14.0`, and that exact mismatch is what silently broke the old `Deploy Web` workflow for three months.

FEATURE(docker): self-host in one command. New root `Dockerfile` (multi-stage, `node:24-bookworm-slim`, standalone output), `docker-compose.yml` (web + worker, healthchecks, `restart: unless-stopped`), `.dockerignore`, and `scripts/setup.sh`. Both images build and the web container answers HTTP 200 with a healthy healthcheck. `app/sitemap.tsx` is prerendered at build time and calls `createAdminClient()`, so `SUPABASE_SERVICE_ROLE_KEY` is passed as a BuildKit secret rather than baked into a layer, with a placeholder fallback so the build never fails. Requires Docker Compose >= 2.23.1.

FIX(worker): `worker/Dockerfile` could not build and was missing runtime dependencies. `RUN playwright install chromium --with-deps` failed because `playwright` is neither in `requirements.txt` nor imported anywhere in `worker/`; removed. `ffmpeg` and `ffprobe` were absent although `telegram_deliverer.py`, `whisper_transcriber.py` and `main.py` all shell out to them; added, along with `curl` for the healthcheck. A new `worker/.dockerignore` cuts the build context from roughly 300 MB to 577 kB by excluding `venv/`, logs and cookies.

CHORE(docs): rewrite `README.md`. The previous one documented Prisma, Better Auth, GitHub OAuth and `pnpm prisma:migrate` — none of which exist in this codebase — so anyone following the Quick Start failed on the first command. The new one covers the real stack, the three-loop worker architecture, and states plainly that the project is maintained casually.

CHORE(repo): add open-source governance — `SECURITY.md` (GitHub Security Advisories, honest response windows), `CODE_OF_CONDUCT.md` (Contributor Covenant 2.1), `.github/ISSUE_TEMPLATE/` (bug report, feature request, config) and `.github/PULL_REQUEST_TEMPLATE.md`. `CONTRIBUTING.md` grew from 82 to 223 lines and had three factual errors fixed: `npm` instead of `pnpm`, a reference to a nonexistent `.env.local.example`, and migrations pointed at an empty `supabase/migrations/`.

CHORE(scripts): make the Supabase endpoint configurable in `health-check.sh`, `auth-smoke-test.sh`, `onboarding-smoke-test.sh` and `backfill-channel-ids.py`. All four hardcoded the hosted project URL, so a contributor running them from a fork would have pointed at production. They now read `BRIEFTUBE_SUPABASE_URL`, then `NEXT_PUBLIC_SUPABASE_URL`, and fall back to the hosted instance — existing cron jobs keep working unchanged. Note the project ref is not a secret: it already appears in the site's HTML and in the `vercel.json` CSP.

CHORE(repo): untrack internal working documents ahead of going public — `.env-template` (dead: declares Better Auth, Neon and GitHub OAuth, none of which the code reads), `SIMPLIFICATION-PLAN.md`, `INTEGRATION-SUPABASE.md`, `_bmad-output/` and `scripts/conductor-*.sh`. Removed with `git rm --cached` and added to `.gitignore`, so they stay on disk locally.
...
FIX(worker): restore Pro/Trial detection on the Telegram bot — `_is_pro()` and `_get_plan_label()` parsed `trial_ends_at` as an ISO string, but `db.get_profile_by_telegram` returns a `datetime` under the psycopg2 bypass, so the `TypeError` was swallowed and every trial user was silently downgraded to Free (gating 5 Pro features). 4 users were on an active trial, 1 of them connected to Telegram. Extracted `_parse_trial_end()` handling both types.
...
FIX(deploy): update-pi.sh no longer aborts before the worker restart when the local Modal deploy fails (`set -e` + typer/click crash silently blocked today's worker update on the Pi); also re-pin click>=8.2 after requirements installs since gtts pins click<8.2 but modal/typer need >=8.2 (gtts only uses click for its unused CLI)
