...
CHORE: blog — remove all 114 em-dashes from src/content/blog.ts across the 19 articles. Replaced each em-dash with the most natural punctuation for its grammatical context (comma for apposition/enumeration, period for clause separation, colon for clarification/list introduction). Em-dashes are a well-known AI-writing tell and were making the blog content feel machine-generated; the rewrite preserves every word but makes the text read like it was written by a human. Regular hyphens in compound words (long-form, high-quality, AI-powered, etc.) were left untouched.
