...
FIX: instrumentation.ts — disable Sentry server-side init (sentry.server.config / sentry.edge.config no longer imported) because Next.js 16.2 + Turbopack standalone build rewrites @opentelemetry/api, require-in-the-middle and friends with content-hashed external module names that fail to resolve at runtime ("Cannot find module '@opentelemetry/api-2543c6b61b192f2f'"); the previously-tried serverExternalPackages workaround only renamed which package crashed, since the hash bug is in Turbopack's external module resolution itself. Client-side Sentry (instrumentation-client.ts) is unaffected and continues reporting browser errors. The 7 web service crashes between 10:16 and 13:01 UTC on 2026-04-07 are now resolved; standalone server starts cleanly past the instrumentation hook (verified locally with `node .next/standalone/server.js`)
...
FIX: Stripe — patch stripe_customer_id null for coolclavier99 (cus_U2BAxiaUz7yQh4) and manually send upgrade confirmation email
