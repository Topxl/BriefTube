...
async def _process_video(
    ...
    try:
        ...
        if cached:
            ...
            # Check permanent archive before hitting YouTube
            archived = transcript_store.load(video_id)
            if archived:
                transcript = archived["text"]
                source_lang = archived["language"]
                transcript_cost = 0.0  # No extraction cost — reusing archive
                error = None
                transcript_extractor.last_transcript_source = archived["source"]
                transcript_extractor.last_ip_blocked = False
                logger.info(f"[{video_id}] Using archived transcript ({len(transcript)} chars, source={archived['source']})")
            else:
            ...
            if transcript:
                ...
                # Archive permanently on disk
                transcript_store.save(
                    video_id, transcript, source_lang,
                    transcript_extractor.last_transcript_source or "unknown",
                    transcript_cost,
                )
